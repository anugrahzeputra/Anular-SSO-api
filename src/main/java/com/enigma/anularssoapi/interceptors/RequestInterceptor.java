package com.enigma.anularssoapi.interceptors;
...
public class RequestInterceptor extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        UsernamePasswordAuthenticationToken userPass = getUserPass(token);
        SecurityContextHolder.getContext().setAuthentication(userPass);
        filterChain.doFilter(request, response);
    }

    private UsernamePasswordAuthenticationToken getUserPass(String token) {
